...
from .models import All_url, Member, Comment_test
...
def winner(request,selected_theme,selected_name,rounds):
    template = loader.get_template('winner.html')
    print("확인용확인용확인용확인용확인용확인용확인용확인용확인용")
    print(rounds)
...
def edit(request,user_id):    
    template = loader.get_template('userinfoedit.html')
    user = Member.objects.get(id=user_id)
    context = {
        'user' : user,
    }
...
def edit_ok(request, user_id):
    a = request.POST['username']
    b = request.POST.get('pwd1', False)
    user = Member.objects.get(id=user_id)
    user.username = a
    user.pwd = b
    user.save()
